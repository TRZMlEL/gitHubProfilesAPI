<template>
  <body class="bg-gray-950 flex items-center justify-center w-screen h-screen text-light bg-gradient-to-br from-gradient1 via-gradient2 to-gradient3 animate-gradient flex-col p-10">
    <div class="flex items-center justify-center w-11/12 md:w-3/5 lg:w-1/3">
    ...
    <div v-if="userData" class=" w-11/12 md:w-3/4 lg:w-1/2 bg-dark flex flex-col p-5 rounded-2xl">
...
<script setup>
  ...
  const getData = async () => {
    ...
    const response2 = await fetch(APIURL + username + '/repos?sort=created'); 
    reposData.value = await response2.json();
  }
</script>